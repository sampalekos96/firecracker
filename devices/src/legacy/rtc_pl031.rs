...
use sys_util::ClockType;
// use sys_util::time::get_time;
//use bus::Error;
...
impl RTC {
    ...
    pub fn new(interrupt_evt: EventFd) -> RTC {
        RTC {
            ...
            tick_offset: sys_util::get_time(sys_util::ClockType::Real) as i64,
            match_value: 0,
            load: 0,
            imsc: 0,
            ris: 0,
    ...
    fn get_time(&self) -> u32 {
        ...
        (ts / sys_util::NANOS_PER_SECOND as i128) as u32
    }
    ...
    fn handle_write(&mut self, offset: u64, val: u32) -> Result<()> {
        match offset {
            ...
            RTCLR => {
                ...
                self.tick_offset = sys_util::seconds_to_nanoseconds(i64::from(val)).unwrap();
            }
            RTCIMSC => {
                self.imsc = val & 1;
