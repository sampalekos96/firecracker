...
pub enum Error {
    ...
    GetCoreRegister(io::Error, String),
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match *self {
            ...
            SetCoreRegister(ref err) => {
                write!(f, "Failed to set register: {}", err)
            }
...
pub fn get_manufacturer_id_from_host() -> Result<u32> {
    ...
    let midr_el1 = fs::read_to_string(midr_el1_path).map_err(|err| {
        Error::GetMidrEl1(io::Error::last_os_error())
    })?;
...
//     )
//     .map_err(|err| Error::SetCoreRegister(err))?;
...
// REMEBER TO ADD WHEN SNAP IS FUCTIONAL
