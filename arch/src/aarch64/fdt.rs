...
use self::vm_fdt::{Error as VmFdtError, FdtWriter, FdtWriterNode};
...
pub fn create_fdt<T: DeviceInfoForFDT + Clone + Debug, S: std::hash::BuildHasher>(
    ...
    // Write FDT to memory.
    let fdt_address = GuestAddress(get_fdt_addr(&guest_mem).try_into().unwrap());
    guest_mem.write_slice_at_addr(fdt_final.as_slice(), fdt_address)?;

    println!("Prin vgw apo create_fdt");
...
fn create_memory_node(fdt: &mut FdtWriter, guest_mem: &GuestMemory) -> Result<()> {
    ...
    let temp = guest_mem.end_addr().offset();

    let mem_size = temp as u64 - super::layout::DRAM_MEM_START + 1;
...
fn create_devices_node<T: DeviceInfoForFDT + Clone + Debug, S: std::hash::BuildHasher>(
    fdt: &mut FdtWriter,
    dev_info: &HashMap<String, T, S>,
) -> Result<()> {
    // Create one temp Vec to store all virtio devices
    let mut ordered_virtio_device: Vec<&T> = Vec::new();

    for (_device_id, info) in dev_info {
