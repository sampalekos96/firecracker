...
// Taken from qemu.
const ARCH_GIC_MAINT_IRQ: u32 = 8;
...
pub fn create_fdt<T: DeviceInfoForFDT + Clone + Debug>(
    ...
    // println!("Prin tin create_cpu_nodes");

    // create_cpu_nodes(&mut fdt, num_cpus)?;
    create_cpu_nodes(&mut fdt, &vcpu_mpidr)?;

    // println!("Prin tin create_memory_node");
    create_memory_node(&mut fdt, guest_mem)?;
    // println!("Prin tin create_chosen_node");
    create_chosen_node(&mut fdt, cmdline)?;
    // println!("Prin tin create_gic_node");
...
fn create_memory_node(fdt: &mut Vec<u8>, guest_mem: &GuestMemory) -> Result<()> {
    let mem_size = guest_mem.end_addr().offset() as u64 - super::layout::DRAM_MEM_START;
